public isolated service class WorkExperienceData {
    ...
    isolated function init(int? work_experience_id = 0, int? person_id = 0, WorkExperience? work_experience = null) returns error? {
        ...
        lock {
            WorkExperience work_experience_raw;
            if (id > 0 && person_id == 0) { // work_experience_id provided, give precedance to that
                work_experience_raw = check db_client->queryRow(
            `SELECT *
...
public isolated service class WorkExperienceEvaluationData {
    ...
    isolated function init(int? work_experience_evaluation_id = 0, WorkExperienceEvaluation? work_experience_evaluation = null) returns error? {
        ...
        if (id > 0) { // work_experience_evaluation_id provided, give precedance to that
            ...
            `SELECT *
            FROM work_experience_evaluation
            WHERE
                id = ${id};`);
